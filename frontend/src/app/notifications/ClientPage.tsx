          ...
          <Card key={template.id} padding="md">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <Switch
                  aria-label={`Enable ${humanType(template.notification_type)}`}
                  label={<span className="sr-only">Enable {humanType(template.notification_type)}</span>}
                  checked={template.is_active}
                  onChange={(e) => toggleMutation.mutate({ id: template.id, isActive: e.target.checked })}
                />
              </div>
            </div>
            ...
            {isEditing && (
              <div className="mt-4 space-y-2">
                <Textarea
                  aria-label={`Edit ${humanType(template.notification_type)} message`}
                  value={editText}
