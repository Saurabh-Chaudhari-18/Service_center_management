...
function InvoiceTemplate({
  ...
  const branch = invoice.branch_details;

  const shopName =
    branch?.name || branch?.organization_name || "SHIVANGI INFOTECH";
      ...
      <AppLayout>
        ...
        <PageShell width="wizard">
          ...
          <form onSubmit={handleSubmit(handlePreview)} className="space-y-6">
            ...
            <ActionBar
              secondary={<Button variant="secondary" type="button" onClick={() => router.push(`/billing/${id}`)}>Cancel</Button>}
              primary={<Button type="submit" leftIcon={<FileText className="w-4 h-4" />}>Preview & Save</Button>}
            />
          </form>
