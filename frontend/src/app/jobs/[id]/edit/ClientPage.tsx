...
import { ActionBar, PageShell } from "@/components/shell";
...
export default function EditJobPage() {
  ...
  if (jobLoading) {
    return (
      <AppLayout>
        <Header title="Edit Job Card" subtitle="Loading job details" />
        <PageShell width="wizard"><LoadingState message="Loading job card…" /></PageShell>
      </AppLayout>
    );
  }
  ...
  if (jobError || !job) {
    return (
      <AppLayout>
        <Header title="Edit Job Card" />
        <PageShell width="wizard"><Alert variant="error">Failed to load job details.</Alert></PageShell>
      </AppLayout>
    );
  }
  ...
  if (!isRole("OWNER", "SUPER_ADMIN")) {
    return (
      <AppLayout>
        <Header title="Edit Job Card" />
        <PageShell width="wizard"><Alert variant="error">You do not have permission to edit this job.</Alert></PageShell>
      </AppLayout>
    );
  }
      ...
      <AppLayout>
        ...
        <PageShell width="wizard">
          <form onSubmit={handleSubmit((d) => mutate(d))} className="space-y-6">
            ...
            <ActionBar
              secondary={<Button type="button" variant="secondary" onClick={() => router.push(`/jobs/${jobId}`)}>Cancel</Button>}
              primary={<Button type="submit" size="lg" leftIcon={<Save className="w-5 h-5" />} isLoading={isPending}>Save Changes</Button>}
            />
          </form>
        </PageShell>
      </AppLayout>
