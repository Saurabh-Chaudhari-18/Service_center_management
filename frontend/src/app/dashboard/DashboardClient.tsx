...
// Interactive dashboard island rendered by the server route entry.
  ...
  const actions = [
    {
      ...
      href: "/jobs/new",
      iconClass: "bg-primary-100 text-primary-700 dark:bg-primary-500/15 dark:text-primary-300",
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
      {actions.map((action) => (
        <Link key={action.label} href={action.href} className="block group">
          <Card padding="sm" className="flex h-full items-center gap-3 transition-colors hover:border-primary-200 dark:hover:border-primary-700">
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${action.iconClass}`}
            >
              {action.icon}
            </div>
            <span className="text-left text-sm font-semibold text-neutral-800 dark:text-neutral-200">
              {action.label}
            </span>
          </Card>
        </Link>
